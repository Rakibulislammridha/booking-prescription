...
- Setup & identity: branches, departments, specialties, users, password_reset_tokens, personal_access_tokens, permissions, roles, model_has_permissions, model_has_roles, role_has_permissions, doctors, doctor_profiles, doctor_specialties, doctor_pad_settings, doctor_compounder, holidays, doctor_leaves, settings
...
Seeded roles per tenant (guard `web`): `hospital_admin`, `doctor`, `receptionist`, `compounder`, `accountant` — enum `App\Domain\Clinic\Enums\Role`, values snake_case, new cases **appended** (the seeder inserts in enum order and a test asserts it). `compounder` is a role of its own and not another name for `receptionist`, which is what this line said before the role existed: it carries four permissions of its own and is restricted to the doctors listed for it in `doctor_compounder` (below) through `App\Domain\Clinic\Services\DoctorScope` — no row there means no patients at all; permission names follow the `<module>.<resource>.<action>` grammar of `App\Domain\Clinic\Enums\Permission` (ARCHITECTURE.md §6.2, the single source of truth — permission strings quoted elsewhere in this document are illustrative); plus `patient` on guard `patient` (assigned to `App\Models\Tenant\Patient`, so `model_type` may be a Patient class). Super Admin is **not** a spatie role — it is the `super` guard on `public.super_admins`. Permission cache key must include the tenant id (`permission.cache.key = "spatie.permission.cache.tenant_{id}"`, set by the tenancy bootstrapper).
...
#### `doctor_compounder`
**Purpose.** Which compounder works which doctor's desk. A user holding the `compounder` role may see and act on exactly the doctors listed here for them — no row means no patients at all. Read on every request by `App\Domain\Clinic\Services\DoctorScope`; never cached, so revoking an assignment takes effect on the next request.

| Column | Type | Null | Default | Meaning |
|---|---|---|---|---|
| doctor_id | bigint | no | | The doctor whose patients this compounder may work |
| user_id | bigint | no | | The staff user; expected to hold the `compounder` role |
| assigned_by_user_id | bigint | yes | | Who assigned it (kept for the trail; null once that account is deleted) |

**PK** id. **FK** doctor_id → doctors(id) ON DELETE CASCADE; user_id → users(id) ON DELETE CASCADE; assigned_by_user_id → users(id) ON DELETE SET NULL. **Unique** (doctor_id, user_id). **Index** (user_id); (assigned_by_user_id). **timestamps: yes**. **Many-to-many on purpose**: one compounder may stand behind two doctors, one doctor may keep two compounders. **Model** none — the pivot is reached through `Doctor::compounders()` / `User::assignedDoctors()`.
...
17. Spatie roles are snake_case — `hospital_admin`, `doctor`, `receptionist`, `compounder`, `accountant` (`compounder` is a distinct role, scoped to named doctors by `doctor_compounder`; it was never a spelling of `receptionist`); Patient is a guard, not a role; Super Admin is central (`super` guard). Permission strings follow the `<module>.<resource>.<action>` grammar of `App\Domain\Clinic\Enums\Permission` (`serials.split.adjust`, `queue.call-next`, `reception.devices.register`, …); older flat spellings are retired.
