...
{
    public function index(Request $request, ActiveBranch $activeBranch, SessionMaterialiser $materialiser, CapacityService $capacity): Response
    {
        $this->authorize('viewAny', DoctorSchedule::class);

        $doctors = Doctor::query()->active()->orderBy('sort_order')->orderBy('name')->get(['id', 'public_id', 'name', 'name_bn', 'slug']);
