 ...
 *
 * Scoped by the caller: a compounder closing their day sees what was issued and collected at their doctor's desk,
 * not the branch's takings and not their colleagues' drawers.
 */
final class ShiftController extends Controller
{
    public function index(Request $request, ActiveBranch $activeBranch, ShiftSummary $summary, DoctorScope $scope): Response
    {
        ...
        return Inertia::render('Reception/Shift', ['summary' => $summary->build($branch, $date, $user === null ? [] : $scope->doctorIds($user))]);
    }
}
